...
export const TargetWeight: React.FC<TargetWeightProps> = ({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        saveOnboardingData({ weight: currentW, target_weight: selected });
        if (!savedRef.current) {
  ...
  const diffMsg =
    diff < 0
      ? {
          badge: "Let's go",
          text: `You will lose ${diffAbs}${unitLabel}`,
          color: COLORS.success,
        }
      : diff > 0
        ? {
            badge: "Let's go",
            text: `You will gain ${diffAbs}${unitLabel}`,
            color: COLORS.primary,
          }
        : {
            badge: "Maintain",
            text: "Keep your current weight",
            color: COLORS.info,
          };
  ...
  const renderItem = ({ item }: { item: number }) => {
    ...
    const opacity =
      dist === 0 ? 1 : dist === 1 ? 0.45 : dist === 2 ? 0.25 : 0.12;
    const inRange = item >= highlightStart && item <= highlightEnd;

    return (
      <View style={[s.item, inRange && !isSel && s.itemInRange]}>
        <Text
          style={[
            s.itemText,
            isSel && s.itemTextSel,
    ...
    <View style={s.root}>
      ...
      <View style={s.toggleWrap}>
        <TouchableOpacity
          ...
          onPress={() => setUnit("kg")}
        >
          <Text style={[s.toggleText, unit === "kg" && s.toggleTextActive]}>
            Kg
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          onPress={() => setUnit("lbs")}
        >
          <Text style={[s.toggleText, unit === "lbs" && s.toggleTextActive]}>
            Lbs
          </Text>
        </TouchableOpacity>
      </View>
